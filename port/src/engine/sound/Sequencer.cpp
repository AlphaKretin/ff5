...
void Sequencer::playSong(const uint8_t* songData, int songLen) {
...
void Sequencer::execCmd(ChannelState& ch, uint8_t cmd, uint8_t p1) {
    ...
    // ── $EA SetSample ───────────────────────────────────────────────────────
    // Loads freqMult from SampleBank metadata for the given instrument index.
    case 0xEA: {
        int id = (int)p1;
        ch.sampleId   = (uint8_t)id;
